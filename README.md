...
<p>The server will run on the port specified in your <code>.env</code> file.</p>
## Running Tests
...
1. **Create a `.env.test` file in the root directory of the project:**
    ...
    This file should contain the necessary environment variables for running the tests. Specifically, ensure you have the `DATABASE_URL` and `PORT` configured for your test environment. You can copy the content from your existing `.env` file and modify the `DATABASE_URL` to use a test-specific database.

    ```dotenv
    DATABASE_URL="postgresql://username:password@localhost:5432/shop-test?schema=public"
    PORT=your-test-port
    ```

    Replace the following placeholders with your actual test database details:
    - `username`: Your PostgreSQL username
    - `password`: Your PostgreSQL password
    - `localhost`: The hostname of your PostgreSQL server (e.g., `localhost`)
    - `5432`: The port number your PostgreSQL server is running on (default is `5432`)
    - `shop-test`: The name of your test database
    - `schema`: The schema you want to use (default is `public`)
    - `your-test-port`: The port number for running the test server (choose an available port, e.g., `4000`)
